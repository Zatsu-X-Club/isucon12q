...
func connectToTenantDB(id int64) (*sqlx.DB, error) {
	...
	p := tenantDBPath(id)
	db, err := sqlx.Open(sqliteDriverName, fmt.Sprintf("file:%s?mode=rw&_journal_mode=WAL&cache=shared&_synchronous=NORMAL", p))
	if err != nil {
...
func billingReportByCompetition2(ctx context.Context, tenantDB dbOrTx, tenantID int64, competiton []CompetitionRow) ([]BillingReport, error) {
	...
	if err := adminDB.SelectContext(
		ctx,
		&vhs,
		"SELECT player_id, MIN(created_at) AS min_created_at, competition_id FROM visit_history WHERE tenant_id = ? GROUP BY competition_id, player_id",
		tenantID,
...
func playersAddHandler(c echo.Context) error {
	...
	if _, err := tenantDB.NamedExecContext(
		...
		return fmt.Errorf("error bulk insert player: %w", err)
	}

	for _, player := range players {
		playerCache.Set(player.ID, nil)
		pds = append(pds, PlayerDetail{
			ID:             player.ID,
